...
// Функция для вывода адреса в консоль
void consoleOutputAddress(void* address) {
    char buffer[32];
    sprintf(buffer, "%p\n", address);
...
int main() {
    ...
    if (hModule == NULL) {
        consoleOutput("Using system allocator\n");
    } else {
        ...
        consoleOutput("Аллокатор создан\n");

        // Выделение памяти
        void* memory = allocator_alloc_ptr(allocator, 128);
        consoleOutput("Память выделена: ");
        consoleOutputAddress(memory);

        // Освобождение памяти
        allocator_free_ptr(allocator, memory);
        consoleOutput("Память освобождена\n");

        // Уничтожение аллокатора
        allocator_destroy_ptr(allocator);
        consoleOutput("Аллокатор уничтожен\n");
